...
#next_rights = {'d--':'b--', 'b--':'a-.', 'a-.':c-.', 'c-.':'d--'}
next_rights = "d--b--a-=c-=d--"
...
def offsetSprite(sprite, xOffset, yOffset):
    print("adjusted sprite points")
    for p in sprite:
        print( xOffset*3+p[0], yOffset*3+p[1])
            
def nuRightTest(y):
    print(next_rights)
    for i in range(4):
        j = i*3
        code = next_rights[j:j+3]
        xOffset = toOffset(code[1])
        yOffset = y*3 + toOffset(code[2])
        print(code, xOffset, yOffset)
        if (code[0]=='a'): sprite = a
        if (code[0]=='b'): sprite = b
        if (code[0]=='c'): sprite = c
        if (code[0]=='d'): sprite = d
        #offsetSprite(sprite, xOffset, yOffset)
        drawe(xOffset, yOffset, sprite)
        oled.show()
